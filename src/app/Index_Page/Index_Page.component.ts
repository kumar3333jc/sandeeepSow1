...
@Component({
  ...
  styleUrls: ['./Index_Page.component.css']
})
export class Index_PageComponent{



  Index_Page_form1:FormGroup;
  Index_Page_form1_submitted=false;
  serviceResData:any;
  configs:any;
  response:any;
  ...
  constructor(private formDataService: FormDataService, private fb: FormBuilder, private router: Router, private service: ServiceInvoker) {
	...
	this.Index_Page_form1 = fb.group({
	});

	this.formDataService.setFormData(this.Index_Page_form1);
	this.configs = new serviceConfigurations();
  }
	

  get formRef1() { return this.Index_Page_form1.controls; }

  Index_Page_form1_onSubmit() {
...
// No services available
  ...
  navigateToComponent(navigatePath: string) {
	this.Index_Page_form1_submitted=true;
	if (this.Index_Page_form1.valid) {
		// Save the form data
		this.formDataService.saveFormData(this.Index_Page_form1.value);
