...
@Component({
  selector: 'TP_Verification',
  templateUrl: './TP_Verification.component.html',
  styleUrls: ['./TP_Verification.component.css']
})
export class TP_VerificationComponent{



  TP_Verification_form1:FormGroup;
  TP_Verification_form1_submitted=false;
  serviceResData:any;
  configs:any;
  response:any;
  ...
  constructor(private formDataService: FormDataService, private fb: FormBuilder, private router: Router, private service: ServiceInvoker) {
	...
	this.TP_Verification_form1 = fb.group({
	});

	this.formDataService.setFormData(this.TP_Verification_form1);
	this.configs = new serviceConfigurations();
  }
	

  get formRef1() { return this.TP_Verification_form1.controls; }

  TP_Verification_form1_onSubmit() {
...
// No services available
  ...
  navigateToComponent(navigatePath: string) {
	this.TP_Verification_form1_submitted=true;
	if (this.TP_Verification_form1.valid) {
		// Save the form data
		this.formDataService.saveFormData(this.TP_Verification_form1.value);
