...
@Component({
  selector: 'tax_details',
  templateUrl: './tax_details.component.html',
  styleUrls: ['./tax_details.component.css']
})
export class tax_detailsComponent{



  tax_details_form1:FormGroup;
  tax_details_form1_submitted=false;
  serviceResData:any;
  configs:any;
  response:any;
  ...
  constructor(private formDataService: FormDataService, private fb: FormBuilder, private router: Router, private service: ServiceInvoker) {
	...
	this.tax_details_form1 = fb.group({
	});

	this.formDataService.setFormData(this.tax_details_form1);
	this.configs = new serviceConfigurations();
  }
	

  get formRef1() { return this.tax_details_form1.controls; }

  tax_details_form1_onSubmit() {
...
// No services available
  ...
  navigateToComponent(navigatePath: string) {
	this.tax_details_form1_submitted=true;
	if (this.tax_details_form1.valid) {
		// Save the form data
		this.formDataService.saveFormData(this.tax_details_form1.value);
